...
// creating QueueHandles for extern use also
...
typedef enum {
	PASSIVE = 0, ATTACKING = 1
} player_bullet_status_t;
...
void UDPHandler(size_t read_size, char *buffer, void *args) {
	...
	pdTRUE) {
		...
		if (strncmp(buffer, "INC", (read_size < 3) ? read_size : 3) == 0) {
			next_key = INC;
			send_command = 1;
		} else if (strncmp(buffer, "DEC", (read_size < 3) ? read_size : 3)
				== 0) {
			next_key = DEC;
			send_command = 1;
		} else if (strncmp(buffer, "NONE", (read_size < 4) ? read_size : 4)
				== 0) {
			next_key = NONE;
			send_command = 1;
		}
		...
		portYIELD_FROM_ISR(
				xHigherPriorityTaskWoken1 | xHigherPriorityTaskWoken2
...
void vUDPControlTask(void *pvParameters) {
	...
	udp_soc_receive = aIOOpenUDPSocket(addr, port, UDP_BUFFER_SIZE, UDPHandler,
			NULL);
	...
	while (1) {
		...
		while (xQueueReceive(PlayerBulletModeXQueue, &player_bullet_status, 0)
				== pdTRUE) {
		}
		...
		// send difference between mothership and player
		if (last_diff != diff) {
...
unsigned char xCheckMothershipUDPInput() {
	...
	if (current_key == INC) {
		mothership.inc = 1;	// moving right
		mothership.dec = 0;
	} else if (current_key == DEC) {
		mothership.inc = 0;
		mothership.dec = 1;	// moving left
	} else if (current_key == NONE) {
		mothership.inc = 0;	// stops moving
		mothership.dec = 0;
	}
	return 0;
}
...
void vSetScoreFlag(unsigned char score_value_flag) {
	// cheat mode: set predefined score
...
void increment_level() {
	if (xSemaphoreTake(game_wrapper.lock, portMAX_DELAY) == pdTRUE) {
		...
		// indicates with set_level_flag that level got manually set
...
void vInitInvaders(double speed) {
	if (xSemaphoreTake(invaders.lock, portMAX_DELAY) == pdTRUE) {
		...
		invaders.paused = 0;
		invaders.killed = 0;
		invaders.speed = speed; // defined from game_wrapper
...
void vInitPlayer(void) {
	if (xSemaphoreTake(player.lock, portMAX_DELAY) == pdTRUE) {
		...
		// Init position of player is in the middle of the screen
...
void vInitMothership(void) {
	if (xSemaphoreTake(mothership.lock, portMAX_DELAY) == pdTRUE) {
		prints("Init mothership.\n");

		mothership.alive = 0;
		mothership.pos_x = -MOTHERSHIP_SIZE_X; // outside of left screen border
		mothership.inc = 0;
		mothership.dec = 0;
		mothership.stop = 0;
	}
		...
		for (short bunkerNumber = 0; bunkerNumber < 4; bunkerNumber++) {
			for (short x = 0; x < NUM_BUNKER_BLOCK_X; x++) {
				for (short y = 0; y < NUM_BUNKER_BLOCK_Y; y++) {
					...
					// init destruction state for each block
					bunker.bunkers[bunkerNumber].block_destruction_state[y][x] =
							3;
				}

				// initialize player and alien front from bunker
				bunker.bunkers[bunkerNumber].player_front[x] =
				NUM_BUNKER_BLOCK_Y - 1;
				bunker.bunkers[bunkerNumber].aliens_front[x] = 0;
			}
			// init x pos
			bunker.bunkers[bunkerNumber].pos_x = SCREEN_WIDTH / 5
					* (bunkerNumber + 1)
					- (BUNKER_BLOCK_SIZE_X * NUM_BUNKER_BLOCK_X) / 2;
		}
		// init y pos
...
void vInitGameWrapper(double *speed) {
	if (xSemaphoreTake(game_wrapper.lock, portMAX_DELAY) == pdTRUE) {
		...
		if (game_wrapper.next_level_flag) {
			...
			game_wrapper.get_extra_life_scores +=
			NEW_LEVEL_GET_EXTRA_LIFE_SCORE_REWARD;
		...
		} else {	// entering game state from lobby

			// check if cheat mode score was set
			...
			// check if cheat mode level was set
			if (game_wrapper.set_level_flag)
				game_wrapper.set_level_flag = 0;
			else
				game_wrapper.level = 0;

			game_wrapper.remaining_life = 3; // define lifes
		...
		*speed = game_wrapper.speed;	// pass speed it to InitInvaders
	}
	xSemaphoreGive(game_wrapper.lock);
}
...
void vInitGameTaks(void *pvParameters) {
	...
	while (1) {
		...
		vTaskDelay(1000);	// wait to avoid state Debounce time conflict
...
void vMoveAlienBullet(bullet_t *bullet, short speed) {
	...
	if (bullet->alive) {
		if (bullet->pos_y < SCREEN_HEIGHT) {
			bullet->pos_y += speed; //move bullet
		} else {
...
void vAlienShoot() {
	...
	// Receive commands from Alien Shooting Task
	if (AlienShootsQueue)
	...
	if (input == 1) {
		if (xSemaphoreTake(invaders.lock, portMAX_DELAY) == pdTRUE) {
			...
			if (invaders.bullet.alive == 0) {
				// unleash bullet from random alien of last row
				unsigned char random_alien = rand() % NUMBER_OF_ALIENS_X;
				if (invaders.enemy[0][random_alien].alive) {
					invaders.bullet.pos_x = invaders.pos_x + ALIEN_SIZE_X / 2
							+ random_alien * ALIEN_DISTANCE - BULLET_SIZE_X / 2;
...
//					prints("player shoots\n");
					if (player.bullet.alive == 0) {
						player.bullet.pos_x = player.pos_x + PLAYER_SIZE_X / 2
								- BULLET_SIZE_X / 2;
...
void vMoveInvaders(unsigned char *invaders_won, TickType_t *last_time) {
	...
	if (xSemaphoreTake(invaders.lock, 0) == pdTRUE) {
		...
		*last_time = current_time;
		...
		// check left and right border to move back and forth
		if (invaders.direction == right &&
				invaders.pos_x + ALIEN_SIZE_X + invaders.last_column_right
				* ALIEN_DISTANCE > RIGHT_BORDER) {
			invaders.pos_y += INVADERS_STEP_IN_Y;
			invaders.speed *= -1;	// change direction
			invaders.direction = left;
		} else if (invaders.direction == left && invaders.pos_x
				+ invaders.last_column_left * (ALIEN_DISTANCE) < LEFT_BORDER) {
			invaders.pos_y += INVADERS_STEP_IN_Y;
			invaders.speed *= -1;	// change direction
			invaders.direction = right;
		}

		// check if invaders reached bottom line/ bunkers
		if (invaders.pos_y + ALIEN_SIZE_Y
				+ ALIEN_DISTANCE * invaders.maxFront> BUNKER_POS_Y) {
			prints("Invaders reached bunkes.\n");
			if (*invaders_won == 0)
				*invaders_won = 1;
		}
...
void vMoveMothership(TickType_t *last_time_mothership,
	...
	if (xSemaphoreTake(mothership.lock, 0) == pdTRUE) {
		// check AI settings from mothership
		...
		if (mothership.alive) {
			...
			// check if mothership reaches end of screen on right hand side
			if (mothership.pos_x > SCREEN_WIDTH) {
				if (mothership.AI_control) {
					// 2PlayerMode: let mothership reappear on other side
			...
			// check if mothership reaches end of screen on left hand side
			...
			if (xTaskGetTickCount()
					- *last_time_mothership> pdMS_TO_TICKS(WAITING_TIME_FOR_MOTHERSHIP)) {
				mothership.alive = 1;
				mothership.pos_x = -MOTHERSHIP_SIZE_X;
			}
		}
...
void vKillAlien(short y, short x, unsigned char *player_won) {
	...
	if (xSemaphoreTake(invaders.lock, portMAX_DELAY) == pdTRUE) {
		if (invaders.enemy[y][x].alive) {
			...
			killed = 1;
			invaders.killed++;	// increment alien killed in total

			// adjust front of column x
			...
			// check where the invaders block ends on bottom side
			invaders.maxFront = 0;
			...
			// increase speed
			if (invaders.direction == right)
				invaders.speed += INVADERS_MAX_SPEED
						/ (NUMBER_OF_ALIENS_X * NUMBER_OF_ALIENS_Y);
	...
	if (killed) {
		...
		if (xSemaphoreTake(game_wrapper.lock, portMAX_DELAY) == pdTRUE) {
			if (y == 0) { 	// Alien Type 1
				...
				game_wrapper.get_extra_life_scores += ALIEN_TYPE_1_SCORE_REWARD
						/ 10;

			} else if (y < 3 && y > 0) { // Alien Type 2

				game_wrapper.score += ALIEN_TYPE_2_SCORE_REWARD;
				game_wrapper.get_extra_life_scores += ALIEN_TYPE_2_SCORE_REWARD
						/ 10;
			} else if (y < NUMBER_OF_ALIENS_Y && y > 2) { // Alien Type 3
				game_wrapper.score += ALIEN_TYPE_3_SCORE_REWARD;
				game_wrapper.get_extra_life_scores += ALIEN_TYPE_3_SCORE_REWARD
						/ 10;
			}
...
void vDestructBunkerBlockPlayerSide(short bunkerNumber, short player_front,
	...
	if (bunker.bunkers[bunkerNumber].block_destruction_state[player_front][xBlock]
			> 0) {
...
void vDestructBunkerBlockInvadersSide(short bunkerNumber, short aliens_front,
		short xBlock) {
	// check bunker block state
	if (bunker.bunkers[bunkerNumber].block_destruction_state[aliens_front][xBlock]
			> 0)
		bunker.bunkers[bunkerNumber].block_destruction_state[aliens_front][xBlock] =
				0;
...
void vPlayerGotHit(unsigned char *invaders_won) {
	...
	if (xSemaphoreTake(game_wrapper.lock, portMAX_DELAY) == pdTRUE) {
		if (!game_wrapper.infinite_life_flag) {
			game_wrapper.remaining_life--;	// player loses a life
//			prints("Player loses a life.\n");

			if (game_wrapper.remaining_life <= 0) {	// check if already dead
//				prints("Player dead.\n");
				if (*invaders_won == 0)
					*invaders_won = 1;
			}
...
void vCheckBulletBulletCollision(short invaders_bullet_pos_x,
	...
	// check if player bullet x hit invaders bullet x
	if (invaders_bullet_pos_x > player_bullet_pos_x - BULLET_SIZE_X
			&& invaders_bullet_pos_x < player_bullet_pos_x + BULLET_SIZE_X) {
		// check if player bullet y hit invaders bullet y
...
void vCheckIfPlayerGotHit(short invaders_bullet_pos_x,
	...
	if (invaders_bullet_pos_y > player_pos_y)
		if (invaders_bullet_pos_x >= player_pos_x - BULLET_SIZE_X
				&& invaders_bullet_pos_x <= player_pos_x + PLAYER_SIZE_X)
...
void vCheckIfBunkerGotHitInvaders(short invaders_bullet_pos_x,
	...
	if (xSemaphoreTake(bunker.lock, portMAX_DELAY) == pdTRUE) {
		...
		if (invaders_bullet_pos_y > bunker.pos_y - BULLET_SIZE_Y
				&& invaders_bullet_pos_y
						< bunker.pos_y
								+ BUNKER_BLOCK_SIZE_Y * NUM_BUNKER_BLOCK_Y) {

			for (short bunkerNumber = 0; bunkerNumber < 4; bunkerNumber++) {
				// check at which bunker
				if (invaders_bullet_pos_x > bunker.bunkers[bunkerNumber].pos_x
						&& invaders_bullet_pos_x < bunker.bunkers[bunkerNumber].pos_x + BUNKER_BLOCK_SIZE_X*NUM_BUNKER_BLOCK_X) {
					for (short xBlock = 0; xBlock < NUM_BUNKER_BLOCK_X;
							xBlock++) {
						// check column of bunker
						if ((invaders_bullet_pos_x
								> bunker.bunkers[bunkerNumber].pos_x
										+ xBlock * BUNKER_BLOCK_SIZE_X
										- BULLET_SIZE_X)
								&& (invaders_bullet_pos_x
										< bunker.bunkers[bunkerNumber].pos_x
												+ BUNKER_BLOCK_SIZE_X
												+ xBlock * BUNKER_BLOCK_SIZE_X)) {
							if (bunker.bunkers[bunkerNumber].block_destruction_state[bunker.bunkers[bunkerNumber].player_front[xBlock]][xBlock]
									> 0)
								vDestructBunkerBlockInvadersSide(bunkerNumber,
										bunker.bunkers[bunkerNumber].aliens_front[xBlock],
										xBlock);
						}
					}
...
void vCheckAliensBulletCollision(unsigned char *invaders_won) {
	...
	if (invaders_bullet_alive) {
		...
		if (player_bullet_alive)
			vCheckBulletBulletCollision(invaders_bullet_pos_x,
					invaders_bullet_pos_y, player_bullet_pos_x,
					player_bullet_pos_y);

		vCheckIfPlayerGotHit(invaders_bullet_pos_x, invaders_bullet_pos_y,
				player_pos_x, player_pos_y, invaders_won);
		vCheckIfBunkerGotHitInvaders(invaders_bullet_pos_x,
				invaders_bullet_pos_y, player_pos_x, player_pos_y);
	}
}
...
void vCheckIfInvadersGotHit(short invaders_pos_x, short invaders_pos_y,
	...
	if (player_bullet_pos_y
			< invaders_pos_y + ALIEN_SIZE_Y
					+ ALIEN_DISTANCE * NUMBER_OF_ALIENS_Y
			&& player_bullet_pos_y > invaders_pos_y - INVADERS_SIZE_Y / 2) {
		prints("block reached\n");
		for (short xAlien = 0; xAlien < NUMBER_OF_ALIENS_X; xAlien++) {
			// for every alien check x
			if ((player_bullet_pos_x
					> invaders_pos_x + xAlien * ALIEN_DISTANCE - BULLET_SIZE_X)
					&& (player_bullet_pos_x
							< invaders_pos_x + ALIEN_SIZE_X
									+ xAlien * ALIEN_DISTANCE)) {
				prints("x fits\n");
				// for every alien whose x the bullet is, check all y alien
				for (short yAlien = invaders_front[xAlien]; yAlien >= 0;
						yAlien--) {
					prints("checks yAlien %d, xAlien %d\n", yAlien, xAlien);
					if ((player_bullet_pos_y
							< invaders_pos_y + ALIEN_SIZE_Y
									+ ALIEN_DISTANCE * yAlien)
							&& (player_bullet_pos_y
									> invaders_pos_y + ALIEN_DISTANCE * yAlien)) {
...
void vCheckIfBunkerGotHitPlayer(short player_bullet_pos_x,
	...
	if (xSemaphoreTake(bunker.lock, portMAX_DELAY) == pdTRUE) {
		// check if bunker reached and not already passed
		if (player_bullet_pos_y
				< bunker.pos_y + BUNKER_BLOCK_SIZE_Y * NUM_BUNKER_BLOCK_Y
				&& player_bullet_pos_y > bunker.pos_y - BULLET_SIZE_Y) {

			for (short bunkerNumber = 0; bunkerNumber < 4; bunkerNumber++) {
				// check at which bunker
				if (player_bullet_pos_x > bunker.bunkers[bunkerNumber].pos_x
						&& player_bullet_pos_x < bunker.bunkers[bunkerNumber].pos_x + BUNKER_BLOCK_SIZE_X*NUM_BUNKER_BLOCK_X) {
					for (short xBlock = 0; xBlock < NUM_BUNKER_BLOCK_X;
							xBlock++) {
						// check column
						if ((player_bullet_pos_x
							...
							if (bunker.bunkers[bunkerNumber].block_destruction_state[bunker.bunkers[bunkerNumber].player_front[xBlock]][xBlock]
									> 0)
								vDestructBunkerBlockPlayerSide(bunkerNumber,
										bunker.bunkers[bunkerNumber].player_front[xBlock],
										xBlock);
						}
					}
...
void vCheckIfMothershipGotHit(short player_bullet_pos_x,
	...
	if (xSemaphoreTake(mothership.lock, portMAX_DELAY) == pdTRUE) {
		...
		if (player_bullet_pos_y < MOTHERSHIP_POS_Y + MOTHERSHIP_SIZE_Y
				&& player_bullet_pos_y > MOTHERSHIP_POS_Y - BULLET_SIZE_Y * 2) {
			if (player_bullet_pos_x > mothership.pos_x - BULLET_SIZE_X
					&& player_bullet_pos_x
							< mothership.pos_x + MOTHERSHIP_SIZE_X)
...
void vCheckPlayerBulletCollision(unsigned char *player_won,
	...
	if (player_bullet_alive) {
		...
		vCheckIfInvadersGotHit(invaders_pos_x, invaders_pos_y,
				player_bullet_pos_x, player_bullet_pos_y, player_won,
				invaders_front);
		vCheckIfBunkerGotHitPlayer(player_bullet_pos_x, player_bullet_pos_y);
		vCheckIfMothershipGotHit(player_bullet_pos_x, player_bullet_pos_y,
				last_time_mothership, AI_control_ON);
	}
...
void vSetLastTimeAfterResume(TickType_t *last_time,
	...
	// reset last time in to have a new start value
	if (xSemaphoreTake(invaders.lock, portMAX_DELAY) == pdTRUE) {
		if (invaders.resume) {
...
void vGameHandlerTask(void *pvParameters) {
	...
	while (1) {
		...
		if (player_won) {
			...
			if (StateQueue)
				xQueueSend(StateQueue, &four_state_signal, 0);
		}
...
int init_space_invaders_handler(void) {
	...
	err_udpcontrol: vTaskDelete(DrawPopUpPageTask);
	...
	err_mothership_lock: vTaskDelete(GameHandlerTask);
	err_GameHandlerTask: vTaskDelete(InitGameTaks);
	err_InitGameTaks:
